---
title: "Projet1 analyse de donnée - Shuaibo Huang & Théophane Guiffant"
author: "Shuaibo Huang & Théophane Guiffant"
date: "2024-11-15"
output: 
  pdf_document:
...
```{r}
# Load the dataset
bike_data <- read_excel("Guiffant_Huang_data.xlsx")
```
\
...
ggplot(bike_data, aes(x = cnt)) +
  geom_histogram(aes(y = after_stat(density)),binwidth = 200, fill = "blue", color = "white", alpha = 0.7) +
...
```{r}
ggplot(bike_data, aes(x = cnt)) +
  geom_histogram(aes(y = after_stat(density)),binwidth = 200, fill = "red", color = "white", alpha = 0.7) +
  geom_density(fill = "green", alpha = 0.5) +
...
```
\
Il semble que weather_code et season sont lié car le p est très faible.
Cependant, plusieurs weather_code ont des fréquences trop faibles. 
\
...
si la météo influence l'emprunt de vélib
...
dépendre des pics horaires lié à l'heure d'emprunt. 
\
...
\newpage
Essayons de créer un nouveau modèle avec le log de cnt. Cela permet d'amoindrir les valeurs
extrèmes.
...
\newpage
Testons la normalité des résidus et l'homscédaticité du modèle.
...
Notre modèle ayant un R2=0.5833 capture une bonne part des variations
