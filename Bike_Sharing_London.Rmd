...
# II. Description de données Metadata
...
Peut-on créer un modèle à partir des variables données pour prédire
l'emprunt de vélo(cnt)?
...
Représentation graphique à l'aide d'un diagramme en barre de la
...
Visualisation de la relation entre t1 et cnt.
...
Calcul du coefficient de corrélation entre t1 et cnt par le test de
Pearson.
...
beau/chaud plus il y a d'emprunt de vélib.
...
Boxplots des distributions des emprunts (cnt) suivant la météo
(weather_code).
...
bike_data<-bike_data%>%
  group_by(weather_code,hour)%>%
  mutate(n=sum(cnt))

ggplot(bike_data, aes(x =cnt,fill=weather_code)) +
  geom_histogram(binwidth = 2, alpha = 0.7) +
  facet_wrap(~weather_code,nrow=8)+
  labs(title = "Emprunts par heure et par saison", x = "hour", y = "nombre d'emprunts") +
  theme_minimal()
```
...
d'emprunt mais le coeff de corrélation nous dit qu'il a une petite
influence.
...
is_holiday+is_weekend+weather_code+wind_speed+season+hum R2 passe a 0.48
mais les résultats qui vont suivre sont les même et la relation est plus
simple)
