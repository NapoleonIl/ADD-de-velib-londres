...
pca1 <- bike_data %>%
  ...
  scale() #centre- normalise

pca_result <- PCA(pca1, graph = T)
fviz_eig(pca_result, addlabels = TRUE, ylim = c(0, 80)) 
get_eig(pca_result) %>% knitr::kable()

fviz_contrib(pca_result, 'ind', axes = 1:2)
...
```{r}
modele_lm <- lm(cnt ~  pca_result$ind$coord[, 1:2] + hour + season ,data = bike_data)
summary(modele_lm)
```
...
```
```{r}
aov(cnt ~ +t1+ weather_code + season,data = bike_data)
```
